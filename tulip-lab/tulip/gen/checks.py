...
def run(preset, token_counts=None):
    ...
    keep = ("id", "lang", "locale", "family", "traps", "answer", "program",
            "holdout", "n_rows", "activity", "format")
    ...
    def light(t):
        ...
        out["shown"] = sorted(T.task_shown_groups(t))
        return out
    ...
    # section 10.3 shares: the kind of a task is drawn once per task from
    # a low-discrepancy sequence, so the shares are met to within a few
    # tasks - no +-30% tolerance: at most 2% of the rate (or 3 tasks, on
    # a small preset) below it, at most 10% of it (or 0.5 point) above
    slack = 3.0 / max(1, len(train))
    for label, rate, where in SHARE_RULES:
        got = sum(1 for t in train if where(t)) / max(1, len(train))
        good = rate - max(slack, 0.02 * rate) <= got <= \
            rate + max(0.005, 0.1 * rate)
        ok3 = ok3 and good
    ...
    for target in ("products", "services"):
        ...
        if rows:
            lines.append(
                "%s rows: >30 %.1f%%, >100 %.1f%%, max %d (3-400, "
                "log-uniform)" % (target, 100 * sum(x > 30 for x in rows) /
                                  len(rows), 100 * sum(x > 100 for x in rows)
                                  / len(rows), rows[-1]))
    hold = T.holdout()
    fams = T.families()
    missing = []
    ...
    act_group = dict((a["n"], T.group_of_activity(a))
    ...
    for split, tasks in splits.items():
        for t in tasks:
            ...
            groups = set(t["shown"])
            for g in (T.group_of_family(t["family"], hold),
                      act_group.get(t.get("activity"), "train")):
                if g != "train":
                    groups.add(g)
    ...
    per_split = ", ".join("%s %d" % (s, sum(
        v for k, v in problems.items() if k.split(" ")[0] == s or
        k.endswith(" " + s))) for s in config.SPLITS)
    check("6 hold-out rules", not problems,
          ("; ".join("%s: %d" % kv for kv in problems.items()) or
           "all kept") + "\nviolations per split (header texts compared "
          "after NFKC + case folding): " + per_split)
    # 7. token lengths
    if token_counts:
