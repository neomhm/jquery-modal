"""
gen/people.py - people, company names, postal addresses and e-mail
addresses for every locale.

Ported from the Extractor's generator (the PLAN shares one set of names
and places): people and places come from the locale's own lists
(gen/data/<locale>/names.json, streets.json, cities.json), written for
the PLAN; Tulip does not use Faker. Addresses follow each country's
order and postcodes.
"""
import re
import unicodedata
...
def core_name(rng, loc, lex, act, city, family_person):
    ...
    words = (lex.get("activity_words") or {}).get(act["id"]) or ["Services"]
    for _ in range(20):
