...
right-to-left marks that Arabic sheets are full of. Every helper that
reads digits reads Arabic-Indic, Persian and Devanagari digits too
(ascii_digits), and full-width ones and full-width punctuation through
NFKC; text() alone keeps a cell's Arabic-Indic digits as written.
...
# Appendix F: the countries of each language, its FIRST locale first.
# A pair not in this table (en-DE, ar-JO, zh-MO...) reads numbers and
# dates like the language's first locale (section 15.2, step 3), but
# keeps its real country for currency and phone.
APPENDIX_F = {
    "ar": ("SA", "AE", "EG", "MA"), "zh": ("CN", "TW", "HK", "SG"),
    "en": ("US", "GB", "IN", "AU", "CA", "NG"),
    "fr": ("FR", "BE", "CH", "CA", "MA", "SN"), "ru": ("RU", "KZ", "BY"),
    "es": ("ES", "MX", "AR", "CO", "CL"), "it": ("IT", "CH"),
    "hi": ("IN",), "ja": ("JP",), "ko": ("KR",)}


def format_country(lang, country):
    """The country whose NUMBER and DATE formats apply: the country
    itself when (lang, country) is an Appendix F pair, else the
    language's first locale's. No country stays None (the helpers' own
    no-country rules apply); an unknown language is left alone."""
    known = APPENDIX_F.get(lang)
    if not known or country is None or country in known:
        return country
    return known[0]
...
SEED_TAX_INCLUDED = {
    # a bare "شامل" / "شاملة" (included) is common in Arabic price
    # headers; the negated forms below are always tested first
    "ar": ["شامل الضريبة", "شامل ضريبة القيمة المضافة", "شامل", "شاملة",
           "يشمل الضريبة", "تشمل الضريبة"],
    "zh": ["含税", "含稅", "价税合计", "價稅合計"],
    ...
    "ko": ["부가세 포함", "VAT 포함"]}
SEED_TAX_EXCLUDED = {
    "ar": ["غير شامل الضريبة", "بدون ضريبة", "غير شامل", "غير شاملة",
           "لا يشمل", "لا تشمل", "لا يشمل الضريبة", "لا تشمل الضريبة"],
    "zh": ["不含税", "未税", "不含稅", "未稅"],
...
def decimal_of(lang, country):
    ...
    if lang is None:
        return None
    country = format_country(lang, country)
    if lang == "ar":
...
# The longest date any rule here can read is about 45 characters
# ("15 de septiembre de 2024 14:30:00 p. m."); a longer cell is not a
# date. Refusing it early keeps date() linear: the trailing-time split
# below backtracks, and cost grew with the SQUARE of the cell (40,000
# characters took 42 s, and the runtime only checks its timeout between
# rows).
MAX_DATE_CHARS = 128
...
def _read_date(text, locale):
    lang, country = parse_locale(locale)
    country = format_country(lang, country)        # en-DE reads as en-US
    t = ascii_digits(clean(text))
    t = single_spaces(t)
    if len(t) > MAX_DATE_CHARS:
        return None
...
def date(value, locale=None):
    ...
    if not isinstance(value, str):
        return FAIL
    if len(value) > MAX_DATE_CHARS and \
            len(single_spaces(clean(value))) > MAX_DATE_CHARS:
        return FAIL                              # not a date (see above)
...
def email(value, locale=None):
    if not isinstance(value, str):
        return FAIL
    t = ascii_digits(clean(value)).lower()
