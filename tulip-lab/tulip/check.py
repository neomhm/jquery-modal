...
It never installs or upgrades anything, and neither does build.py: the
small pure-Python packages (openpyxl, et_xmlfile, phonenumbers,
hijridate) come from the wheels in vendor/ (config.use_vendor), and torch
is never touched (MEGA9 has a ROCm build and the desktop a CUDA build,
and replacing either would break it). The MEGA9 /train run has no
network at all.
"""
import importlib
import json
...
import config                  # noqa: E402  (puts vendor/ wheels on the path)
...
def torch_facts():
    ...
    torch.set_num_threads(config.threads())      # never one per core
...
def collect():
    ...
    env = {
        ...
        "workers": config.workers(), "torch_threads": config.threads(),
        "vendor": [pathlib.Path(w).name for w in config.VENDORED],
...
def show(env):
    ...
    rows = [
        ...
        ("CPUs", "%s (this build uses %s processes, %s torch threads)" % (
            env["cpu_count"], env.get("workers"), env.get("torch_threads"))),
        ("vendor/", ", ".join(env.get("vendor") or []) or "none"),
        ("RAM", "%s GB" % env["ram_gb"]),
